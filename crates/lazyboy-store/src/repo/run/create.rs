...
/// Queue a new run. `task_id` is `None` for a chat turn (a run with no
/// task) and `Some` for a task-backed run (a workflow). The Goose session
/// id is unknown until the bridge opens the session, so it starts NULL and
/// is filled by `set_session`.
pub async fn create(
    store: &Store,
    space_id: Id<Space>,
